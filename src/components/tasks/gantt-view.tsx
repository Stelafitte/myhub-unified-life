...
export function GanttView({ tasks, onEdit, onUpdateRange }: Props) {
  ...
  return (
    <div className="rounded-xl border bg-card">
      <div className="flex items-center justify-between border-b px-2 py-2 sm:px-3">
        <div className="text-[10px] text-muted-foreground sm:text-xs">
          {start.toLocaleDateString("fr-FR")} → {end.toLocaleDateString("fr-FR")} · {tasks.length} tâche{tasks.length > 1 ? "s" : ""}
        </div>
        <div className="flex items-center gap-1">
          <Button size="sm" variant={zoom === "day" ? "default" : "outline"} className="h-6 px-1.5 text-[10px] sm:h-7 sm:px-2 sm:text-xs" onClick={() => setZoom("day")}>Jour</Button>
          <Button size="sm" variant={zoom === "week" ? "default" : "outline"} className="h-6 px-1.5 text-[10px] sm:h-7 sm:px-2 sm:text-xs" onClick={() => setZoom("week")}>Sem.</Button>
          <Button size="sm" variant={zoom === "month" ? "default" : "outline"} className="h-6 px-1.5 text-[10px] sm:h-7 sm:px-2 sm:text-xs" onClick={() => setZoom("month")}>Mois</Button>
        </div>
      </div>

      <div ref={containerRef} className="flex max-h-[calc(100vh-10rem)] overflow-auto sm:max-h-[calc(100vh-12rem)]">
        {/* Left labels */}
        <div className="sticky left-0 z-10 shrink-0 border-r bg-card" style={{ width: LABEL_W }}>
          ...
          {groups.map(([section, items]) => (
            <div key={section}>
              <div className="border-b bg-muted/50 px-2 py-1 text-[10px] font-semibold uppercase tracking-wider text-muted-foreground sm:px-3">
                {section}
              </div>
              {items.map((t) => (
                <div key={t.id} className="flex items-center border-b px-2 text-[11px] sm:px-3 sm:text-xs" style={{ height: ROW_H }}>
