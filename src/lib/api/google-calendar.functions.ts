...
async function deduplicateCalendarEventsForUser(userId: string): Promise<number> {
...
export const syncGoogleCalendarEvents = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context }) => {
    ...
    try {
      ...
      console.warn("Calendar pre-sync deduplication skipped", e);
    }
    ...
    if (!connections || connections.length === 0) {
      return { synced: 0, pushed: 0, deduped: totalDeduped, connections: 0 };
    }
